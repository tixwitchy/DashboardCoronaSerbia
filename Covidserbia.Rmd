---
title: "COVID-19 Srbija - Statistika na dan"
output: 
...
covidserbialinechart$Datum<-format(as.Date(covidserbialinechart$Datum),"%d.%m.")
...
covidserbialinechart<-covidserbialinechart%>%
  mutate(Info=paste("<br>","Grad:",NazivTeritorije, "<br>","Datum:",Datum, "<br>","Broj slučajeva:",`sum(Vrednost)`, "<br>"))

covidserbialinecharteng<-covidserbialinechart%>%
  mutate(Info=paste("<br>","Town:",NazivTeritorije, "<br>","Date:",Datum, "<br>","Number of Cases:",`sum(Vrednost)`, "<br>"))
...
valueBox(format(scrapingdead[28], big.mark = ","),color = "black")
```
...
m<-leaflet(covidserbiamap) %>% 
  addTiles(attribution = paste(
    "&copy; <a href=\"http://openstreetmap.org\">OpenStreetMap</a> contributors","Data from: <a href=\"https://data.gov.rs/sr/datasets/covid-19-zarazheni/\">Open data portal Serbia</a>"))%>%
...
filter <- bscols(
  filter_select("nesto", "Izaberi grad", tx, ~NazivTeritorije),
  ggplotly(gg, dynamicTicks = TRUE,tooltip = "label",height = 450,width = 695),
  widths = c(12, 12)
)
filter$x$layout$margin$b <- filter$x$layout$margin$b + 60
...
Row {data-height=815} 
-----------------------------------------------------------------------
### **Map with Total Number of Infected by Towns** 
...
m<-leaflet(covidserbiamap) %>% 
  ...
  setView(lng = 21.0059, lat = 44.0165, zoom = 7)%>%
  addAwesomeMarkers( icon = icon2, popup = ~paste(
    "Town:", `NazivTeritorije`, "<br>","Number of Cases:",`sum(Vrednost)`, "<br>"))

m
```

### **Number of New Cases - Towns by Day**

```{r}

tx1 <- highlight_key(covidserbialinecharteng)
gg1 <- ggplot(tx1,aes(Datum, `sum(Vrednost)`, group = NazivTeritorije,label = Info, color=NazivTeritorije)) +
  geom_point()+
  geom_line()+
  #geom_jitter(height = 0.05)+
 #scale_color_viridis(discrete = TRUE) +
...
filter1 <- bscols(
  filter_select("id1", "Choose Town", tx1, ~NazivTeritorije),
  ggplotly(gg1, dynamicTicks = TRUE,tooltip = "label",height = 450,width = 695),
  widths = c(12, 12)
)

filter1$x$layout$margin$b <- filter1$x$layout$margin$b + 60
filter1
