...
url1<-"https://data.gov.rs/sr/datasets/r/1977833a-36f2-445e-916a-ef51b217d3a0"
...
covidserbialinechart<-covidserbialinechart%>%
  mutate(Info=paste("<br>","Grad:",NazivTeritorije, "<br>","Datum:",Datum, "<br>","Broj slucajeva:",`sum(Vrednost)`, "<br>"))
...
valueBox(format(42,big.mark = ","),color = "green")
```

### **Ukupan broj preminulih**
```{r}

valueBox(format(scrapingdead[29], big.mark = ","),color = "black")
```
...
### **Broj novih slucajeva - gradovi po danima**

```{r}

tx <- highlight_key(covidserbialinechart)
gg <- ggplot(tx) + 
  geom_line(aes(Datum, `sum(Vrednost)`, group = NazivTeritorije,label = Info, color=NazivTeritorije))+
  ylab("Broj novih slucajeva")+
  xlab("")+
...
filter <- bscols(
  ...
  ggplotly(gg, dynamicTicks = TRUE,tooltip = "label"),
  widths = c(12, 12)
)
